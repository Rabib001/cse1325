    ...
    public void solve(int threadID) {
        ...
        while (true) {
            synchronized (counterLock) {
                // Get the next puzzle to work on and increment the counter
                nextPuzzleIndex = sharedCounter;
                sharedCounter++;
    
                // Check if all work has been allocated
                if (nextPuzzleIndex >= NUM_PUZZLES) {
                    break; // No more work to do, exit the loop
                }
            }
    
            // Work on the puzzle at nextPuzzleIndex
            Puzzle p = puzzles.get(nextPuzzleIndex);
        ...
        // Synchronize to add solutions to the shared solutions set
        synchronized (solutions) {
            solutions.addAll(threadSolutions);
        }
    
        System.err.println("Thread " + threadID + " finished.");
    }
